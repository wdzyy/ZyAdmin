<template>
	<div class="layout-navbars-breadcrumb-user pr-15px" :style="{ flex: layoutUserFlexNum }">
		...
		<el-popover ref="userNewsRef" :persistent="false" placement="bottom" transition="el-zoom-in-top"
			trigger="click" :virtual-ref="userNewsBadgeRef" virtual-triggering :width="300">
			<UserNews />
		</el-popover>
		<div class="layout-navbars-breadcrumb-user-icon mr-10px" @click="onScreenfullClick">
			<i class="iconfont" :class="!state.isScreenfull ? 'icon-fullscreen' : 'icon-tuichuquanping'"
				:title="state.isScreenfull ? '关全屏' : '开全屏'"></i>
		</div>
		<el-dropdown :hide-timeout="50" :show-timeout="70" @command="onHandleCommandClick">
			<span class="layout-navbars-breadcrumb-user-link">
				<img class="layout-navbars-breadcrumb-user-link-photo mr-5px" :src="userInfos.photo" />
...
const onHandleCommandClick = (path: string) => {
	if (path === 'logOut') {
		...
		window.open('https://github.com/wdzyy/ZyAdmin');
	} else if (path === 'blog') {
		window.open('https://wdzyy.github.io/zyy/');
	} else {
		router.push(path);
	}
};
