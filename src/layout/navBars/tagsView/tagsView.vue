	...
	<div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
		<el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
			<ul ref="tagsUlRef" class="layout-navbars-tagsview-ul" :class="setTagsStyle">
				<li v-for="(v, k) in state.tagsViewList" :key="k" :ref="(el) => {
						if (el) tagsRefs[k] = el;
					}
						" class="layout-navbars-tagsview-ul-li"
					:class="{ 'is-active': isActive(v) }" :data-url="v.url"
					@click="onTagsClick(v, k)" @contextmenu.prevent="onContextmenu(v, $event)" @mousedown="onMousedownMenu(v, $event)">
					<i v-if="isActive(v)" class="iconfont icon-webicon318 layout-navbars-tagsview-ul-li-iconfont"></i>
					<SvgIcon v-if="!isActive(v) && getThemeConfig.isTagsviewIcon" class="pr5" :name="v.meta.icon" />
					<span>{{ setTagsViewNameI18n(v) }}</span>
					<template v-if="isActive(v)">
						<SvgIcon class="ml-5px layout-navbars-tagsview-ul-li-refresh" name="ele-RefreshRight"
							@click.stop="refreshCurrentTagsView($route.fullPath)" />
						<SvgIcon v-if="!v.meta.isAffix" class="layout-navbars-tagsview-ul-li-icon layout-icon-active" name="ele-Close"
							@click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)" />
					</template>
					<SvgIcon v-if="!v.meta.isAffix" class="layout-navbars-tagsview-ul-li-icon layout-icon-three" name="ele-Close"
						@click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)" />
				</li>
			</ul>
		</el-scrollbar>
...
const transUrlParams = (v: RouteItem) => {
	...
	if (v.meta?.isDynamic) {
		 ...
		 * 2、通过事件调用时，路由的 path 不是原始定义的路由格式，如：/params/dynamic/details/Zy-Admin/111/我是动态路由测试tagsViewName(非国际化)
		 *
