...
// 设置菜单高亮位置移动
function setColumnsAsideMove(k: number) {
  if (k === undefined)
    return false
  state.liIndex = k
...
// 菜单高亮点击事件
function onColumnsAsideMenuClick(v: RouteItem) {
...
// 鼠标移入时，显示当前的子级菜单
function onColumnsAsideMenuMouseenter(v: RouteRecordRaw, k: number) {
...
// 鼠标移走时，显示原来的子级菜单
async function onColumnsAsideMenuMouseleave() {
  if (!themeConfig.value.isColumnsMenuHoverPreload)
    return false
...
// 设置高亮动态位置
function onColumnsAsideDown(k: number) {
  nextTick(() => {
    setColumnsAsideMove(k)
  })
}
// 设置只有一个路由时设置自动收起菜单
function setMenuAutoCollaps(path: string) {
  const resData: MittMenu = setSendChildren(path)
...
// 传送当前子级数据到菜单中
function setSendChildren(path: string) {
...
function setColumnsMenuHighlight(path: string) {
    ...
    <el-scrollbar>
      <ul @mouseleave="onColumnsAsideMenuMouseleave()">
        <li v-for="(v, k) in state.columnsAsideList" :key="k" :ref="(el) => {
          ...
          :title="v.meta.title" @click="onColumnsAsideMenuClick(v)" @mouseenter="onColumnsAsideMenuMouseenter(v, k)">
