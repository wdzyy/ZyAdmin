...
// 关闭赞助商
function onCloseSponsors() {
	state.sponsors.isShow = false
}
// 轮播图改变时
function onCarouselChange(e: number) {
	state.sponsors.index = e
}
// 当前项内容点击
function onSponsorsClick() {
...
<template>
	<div v-show="state.sponsors.isShow" class="sponsors-container" title="点击前往体验" @click="onSponsorsClick">
		<el-carousel :arrow="setCarouselShow" height="240px" indicator-position="none" @change="onCarouselChange">
			<el-carousel-item v-for="(v, k) in state.sponsors.list" :key="k">
				<img class="sponsors-img" :src="v.url">
