...
// 当前 icon 图标点击时
function onColClick(v: unknown | string) {
  emit('get-icon', v)
}
</script>
  ...
  <div class="icon-selector-warp-row">
    <el-scrollbar ref="selectorScrollbarRef">
      <el-row v-if="props.list.length > 0" :gutter="10">
        <el-col v-for="(v, k) in list" :key="k" :lg="4" :md="4" :sm="4" :xl="4" :xs="6" @click="onColClick(v)">
          <div class="icon-selector-warp-item" :class="{ 'icon-selector-active': prefix === v }">
            <SvgIcon :name="v" />
          </div>
        </el-col>
      </el-row>
      <el-empty v-if="list.length <= 0" :description="empty" :image-size="100" />
    </el-scrollbar>
  </div>
</template>
